...
| High | `axios` | CVE-2026-44492 / GHSA-pjwm-pj3p-43mv | `package-lock.json` |
| High | `axios` | Multiple related axios advisories from 2026-05-28 through 2026-06-09 | `package-lock.json` |

## What Needs To Be Done

- Review Dependabot alerts for `axios`, `shell-quote`, and `tmp`.
- Update the dependency chains that bring in those packages.
...
- Start with the latest Dependabot alert for `axios`; one update may resolve several related advisories.
- Use `npm audit` after the dependency updates to verify no advisory remains.
- If axios is only transitive, update the parent package rather than pinning a nested dependency manually.
